...
function JValidator(aValidatorName, aComponentToValidate, anOwnMessage, aGroupMessage, isGrouped, shooters)
{
	if (isGrouped == 'false')
	...
	this.mustValidate= function ()
	{
		var validate= this.shooters.length == 0;
		for (var i= 0; i < this.shooters.length; i++)
			validate= validate || this.shooters[i] == theLastEvent.getSource();
		return validate;
	}
