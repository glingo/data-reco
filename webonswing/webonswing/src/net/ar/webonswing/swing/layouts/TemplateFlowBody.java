...
public class TemplateFlowBody implements TemplateBody
{
	protected StringBuffer theReplacedBodyString= new StringBuffer();

	public void replace(java.util.List theElements)
	{
		theReplacedBodyString= new StringBuffer();

		for (Iterator i= theElements.iterator(); i.hasNext();)
			theReplacedBodyString.append(new ContentRenderer(((DefaultTemplateElement) i.next()).getContent()).getResult());
	}
