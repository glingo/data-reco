...
public class TextFieldUIContributor extends AbstractSwingComponentUIContributor
{
	public void doRenderingContribution(RenderingContributionContainer theContribManager)
	{
		JTextField aTextField= (JTextField) getJComponent();

		Tag theTag= new Tag("input");
		theTag.setNeedsClosure(false);

		theTag.addAttribute(new TagAttribute("type", "text"));
		theTag.addAttribute(new TagAttribute("name", theComponent.getName()));
		theTag.addAttribute(new TagAttribute("value", aTextField.getText()));

		if (aTextField.getColumns() > 0)
			theTag.addAttribute(new TagAttribute("size", aTextField.getColumns()));

		if (aTextField.getWidth() > 0 && aTextField.getHeight() > 0)
			theTag.addAttribute(new TagAttribute("style", "position:absolute; width:" + aTextField.getWidth() + "; height:" + aTextField.getHeight()));

		if (!aTextField.isEnabled())
			theTag.addAttribute(new TagAttribute("DISABLED", ""));

		Template theTemplate= WosSwingHelper.getTemplateForComponent("JTextField", aTextField);
