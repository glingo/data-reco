...
public class StoryPanelContributor extends PanelUIContributor 
{
	public void doPersistenceContribution(PersistenceContributionContainer aPersistenceManager)
	{
		aPersistenceManager.persistValue(theComponent, ((CompleteStoryPanel) getJComponent()).getStory().getId());
	}
	...
	public void restorePersistedValue(PersistenceContributionContainer aPersistenceManager)
	{
		try
		{
			...
			((CompleteStoryPanel) getJComponent()).setStory(WebLogSystem.getStoryStore().getStory(storyId));
			((CompleteStoryPanel) getJComponent()).initialize();
		}
		catch (Exception e)
		{
			e.printStackTrace();
